...
for market, file in markets.items():
    ...
    if market == "US Bonds":
        ...
        X_train_price_train, X_train_price_test, y_train_price_train, y_train_price_test = train_test_split(
            X_train_price, y_train_price, test_size=0.2, shuffle=False)
        
        X_train_volume_train, X_train_volume_test, y_train_volume_train, y_train_volume_test = train_test_split(
            X_train_volume, y_train_volume, test_size=0.2, shuffle=False)
        
        model_price = MLPRegressor(**best_params)
        model_volume = MLPRegressor(**best_params)
        
        model_price.fit(X_train_price_train, y_train_price_train)
        model_volume.fit(X_train_volume_train, y_train_volume_train)
        ...
        for i in range(1, len(future_df)):
            if future_df.iloc[i]['month'] != future_df.iloc[i-1]['month']:
                ...
                future_df.loc[future_df.index[i], 'Price'] = future_df.iloc[i-1]['Price'] * np.random.uniform(0.98, 1.02)
